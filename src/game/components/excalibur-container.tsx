...
export type ExcaliburOptions = Omit<
  EngineOptions,
...
export const ExcaliburContainer = ({
  ...
  useEffect(() => {
    ...
    if (!canvasElement) {
      throw new Error(
        "canvasRef.current should always return HTMLCanvasElement."
      );
    }
    ...
    return (): void => {
      cleanup.set(instanceId, Number(setTimeout(() => engine.stop(), 100)));
    };
  ...
  return <canvas id={instanceId} ref={canvasRef}></canvas>;
};
